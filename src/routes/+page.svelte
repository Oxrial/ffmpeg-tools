<script>
  ...
  async function confirmAndMerge() {
    try {
      await invoke('generate_filelist_and_merge', { files: sortedFiles.filter(s=>!s.delete).map(s => s.id), folderPath });
      alert('视频合并完成！');
    } catch (error) {
      alert(`Error: ${error}`);
    }
      ...
      {#each sortedFiles as file, index (file.id)}
        <SortableItem {...file}  {index}>
          <li>
            ...
            <button on:click={() => {
              file.delete ? undoDelete(index): deleteFile(index)}}>{file.delete ?'撤销':'删除'}</button>
          </li>
        </SortableItem>
      {/each}
    </SortableList>
